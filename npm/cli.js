...
const GITHUB_REPO = 'jeanlopezxyz/mcp-redhat-cases';
const JAR_NAME = 'mcp-redhat-cases.jar';
const CACHE_DIR = path.join(require('os').homedir(), '.cache', 'mcp-redhat-cases');
...
EXAMPLES:
  ...
  npx mcp-redhat-cases

  # SSE mode on port 9080
  npx mcp-redhat-cases --port 9080
...
async function getLatestRelease() {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      headers: { 'User-Agent': 'mcp-redhat-cases' }
    };
...
function downloadFile(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    const request = (url) => {
      https.get(url, { headers: { 'User-Agent': 'mcp-redhat-cases' } }, (res) => {
...
async function getJar(verbose = true) {
  ...
  try {
    ...
    if (verbose) console.error(`Downloading mcp-redhat-cases ${latestVersion}...`);
